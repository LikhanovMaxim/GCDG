...
//const string FILE_NAME = "ExampleSentence.txt";
const string CATEGORIES_OF_WORDS = "ExampleCategories1 a^n b^n c^n.txt";
const string FILE_NAME = "ExampleSentence1_1.txt";
//const string FILE_NAME = "ExampleSentence1_2.txt";
//const string FILE_NAME = "ExampleSentence1_3.txt";
...
public:
	...
	void calcFailFuncR(){
		...
		for (int v = 0; v < valenceR.size(); v++){
			...
			for (int i = 0; i < n; i++){
				...
				cout << i << " " << "word: '" << words[n - (i + 1)] << "'. getCp:" << endl;
				printCategory(cP);
				for (int j = 0; j < cP.rules.size(); j++){ //cP = C^p
	...
	void printItem(Item item){
		...
		if (item.itemL != NULL &&  item.itemR != NULL){
			cout << ", " << item.itemL->number << ", " << item.itemR->number;
		}
		cout << ">" << endl;
	}
